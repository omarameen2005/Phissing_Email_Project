...
from flask import Blueprint, json, render_template, request, jsonify, abort  # NEW: abort
from datetime import datetime
from engine.processor import process_email
from engine.logger import get_recent_logs, get_stats, get_conn  # NEW: Use DB functions

main = Blueprint("main", __name__)

@main.route("/", methods=["GET", "POST"])
def index():
    stats = get_stats()  # FIXED: Use DB
    logs = get_recent_logs(20)  # FIXED: Use DB, last 20

    return render_template(
        "index.html",
        logs=logs,
        stats=stats,
        datetime=datetime  # FIXED: Pass datetime if needed
    )
...
def logs_page():
    stats = get_stats()  # FIXED: Use DB
    logs = get_recent_logs(1000)  # FIXED: All logs, reversed

    return render_template("logs.html", logs=logs, stats=stats)  # FIXED: No reverse, since get_recent_logs is DESC
